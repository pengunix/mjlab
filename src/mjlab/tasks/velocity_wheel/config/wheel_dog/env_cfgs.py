"""Wheel dog velocity environment configurations."""

import math
from typing import Literal

from mjlab.asset_zoo.robots import (
  WHEEL_DOG_ACTION_SCALE,
  get_wheel_dog_robot_cfg,
)
...
from mjlab.tasks.velocity_wheel.velocity_wheel_env_cfg import make_velocity_env_cfg

TerrainType = Literal["rough", "obstacles"]


def wheel_dog_rough_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  """Create Wheel Dog rough terrain velocity configuration."""
  ...
  cfg.scene.entities = {"robot": get_wheel_dog_robot_cfg()}

  foot_names = ("FR", "FL", "RR", "RL")
  site_names = ("imu",)
  geom_names = tuple(f"{name}_wheel_collision" for name in foot_names)
  ...
  base_head_ground_cfg = ContactSensorCfg(
    ...
    primary=ContactMatch(
      mode="geom",
      entity="robot",
      pattern=("base_collision", "head_collision"),
    ),
    secondary=ContactMatch(mode="body", pattern="terrain"),
    fields=("found", "force"),
    reduce="none",
  ...
  joint_pos_action.scale = WHEEL_DOG_ACTION_SCALE
  ...
  cfg.rewards["base_head_collision"] = RewardTermCfg(
    func=mdp.self_collision_cost,
    weight=-0.1,
    params={"sensor_name": base_head_ground_cfg.name},
  )
...
def wheel_dog_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create Wheel Dog flat terrain velocity configuration."""
  cfg = wheel_dog_rough_env_cfg(play=play)
  ...
  for key in ("self_collisions", "shank_collision", "base_head_collision"):
